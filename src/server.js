...
app.get('/api/matches', async (req, res) => {
  try {
    ...
    const response = await axios.get('https://api.gotinder.com/v2/matches?count=1', {
      headers: {
      ...
      for (const message of messageList) {
        ...
        while (!stop) {
          ...
          if (response.status === 200) {
            ...
            allUsers.push(...data.map(user => ({ ...user, hasMessage: !!message })));
            ...
            console.error(`Failed to get data from external API: ${response.status}`);
            stop = true;
          }
        }
      }
  ...
  app.post('/api/deleteMatches', async (req, res) => {
    ...
    for (const matchId of matchIds) {
        ...
        try {
            ...
            return res.status(500).json({ error: `Failed to delete match ${matchId}` });
        }
    }
